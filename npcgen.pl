...
#partyname, members, charfname, charlname, race, class, level, ac(10 + 1/2 level+armor+abilmod+classmod+featmod+enhmod+miscmod+miscmod), atks, dmg, mv, gender, al, st, in, wi, dx, co, ch, hp, bloodied hp(1/2 hp), surge value(1/4 hp), surgesperday, second wind(once per encounter - temp hp), role, skills, save_fort(10+1/2 level+abil+class+feat+enh+misc+misc), save_will, save_reflex, action points, spells, mnr
#
$passes = 0;
...
# race=(dra,dwf,eld,elf,hle,hlf,hum,tfl);
# class=(clr,ftr,pal,rng,rog,wlk,wld,wiz);
# al=(g,lg,e,ce,n);
# diety_g=(avandra,pelor);
# diety_lg=(corellon,erathis,ioun,kord,melora,the raven queen, sehanine);
